...
void plotScan(TString const indir, TString const strxvar, TString const stryvar="deltaNLL", TString const strachypo=""){
  ...
  if (stryvar=="deltaNLL"){
    TString limits_name=indir+"/limits.log";
    ofstream limits(limits_name.Data());
    streambuf* coutbuf = cout.rdbuf();
    cout.rdbuf(limits.rdbuf());

    TSpline3 sp("", gr, "b2e2", 0, 0);
    vector<float> list_cl95, list_cl68;
    for (int ip=0; ip<gr->GetN()-1; ip++){
      for (unsigned int ie=0; ie<2; ie++){
        if (
          (gr->GetY()[ip+1]>=excl[ie] && (gr->GetY()[ip]<excl[ie] || (ip==0 && gr->GetY()[ip]==excl[ie])))
          ||
          (gr->GetY()[ip+1]<=excl[ie] && (gr->GetY()[ip]>excl[ie] || (ip==0 && gr->GetY()[ip]==excl[ie])))
          ){
          float diffy=-1;
          float locx;
          for (int j=0; j<=100; j++){
            float xx=gr->GetX()[ip] + float(j)/float(100)*(gr->GetX()[ip+1]-gr->GetX()[ip]);
            float yy=sp.Eval(xx);
            if (diffy<0. || diffy>fabs(yy-excl[ie])){
              locx = xx;
              diffy = fabs(yy-excl[ie]);
            }
          }
          switch (ie){
          case 0:
            list_cl68.push_back(locx);
            break;
          case 1:
            list_cl95.push_back(locx);
            break;
          }
        }
      }
    }
    for (auto const& cl:list_cl68) cout << "Excl 68%: " << cl << endl;
    for (auto const& cl:list_cl95) cout << "Excl 95%: " << cl << endl;
  ...
  float minY=0, maxY=-1;
  for (int ip=0; ip<gr->GetN(); ip++) maxY=std::max(maxY, (float) gr->GetY()[ip]);
...
void plotAll(TString const indir, TString const strxvar, TString const strachypo=""){
  TChain* tree=new TChain("limit");
  vector<TString> lsdirs = lsdir("./");
  for (auto const& strdir:lsdirs){
    if (strdir.Contains(indir)){
  ...
  delete tree;
  for (auto& bname:bnames) plotScan(indir, strxvar, bname, strachypo);
}
