...
run(){
  ...
  cmd="./makeDatacards_ZZTo2L2Nu.run.sh $date $hypo $year $tpltag $dcinputstag $datainputtag $hasBoostedVH"
  $cmd
  RUN_STATUS=$?
...
for hasBoostedVH in 0; do
  for hypo in SM a2 a3 L1; do
    ...
    if [[ "$hypo" == "SM" ]]; then
      ./buildCards.run.sh hto2l2nu_allcats_13TeV.txt hto2l2nu_allcats_13TeV_RVoverRF.root 13 "${hypo}.UseRVoverRF"
    fi
    for year in 2016 2017 2018; do
