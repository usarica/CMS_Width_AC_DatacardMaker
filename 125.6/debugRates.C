...
void debugRates()
{
  ...
  double masses[1] = {125.6};

  int sqrts[2] = {7 , 8};

  ofstream outfile;

  outfile.open("output_legacy_yields.txt");
      ...
      for (int e=0; e < 2; e++)
	{
	  ...
	  for(int c=1; c < 4; c++)
	      ...
	      else if (c==1)
		{
	      ...
	      TFile* f_new_untagged = new TFile(Form("hzz4l_%sS_%dTeV_0.input.root",channel.c_str(),sqrts[e]),"OPEN");
	      ...
	      w_new_untagged->var("MH")->setVal(masses[i]);
	      ...
	      new_untagged.open(Form("hzz4l_%sS_%dTeV_0.txt",channel.c_str(),sqrts[e]));
	      char line[256];
	      ...
	      for(int n_line=0; n_line < 3; n_line++)
		{
	      ...
	      TFile* f_new_tagged = new TFile(Form("hzz4l_%sS_%dTeV_1.input.root",channel.c_str(),sqrts[e]),"OPEN");
	      ...
	      w_new_tagged->var("MH")->setVal(masses[i]);
	      ...
	      new_tagged.open(Form("hzz4l_%sS_%dTeV_1.txt",channel.c_str(),sqrts[e]));
